...
<div className="flex flex-col h-full px-8 md:px-12 lg:px-16 pt-16 md:pt-32 pb-margin-page">
{/* Brand Mark (Top Row) */}
<div className="mb-12">
<div className="flex items-center gap-3">
<div className="w-10 h-10 bg-secondary flex items-center justify-center rounded-sm">
<span className="material-symbols-outlined text-surface" style={{ "fontVariationSettings": "'FILL' 1" }}>hub</span>
</div>
...
{/* Form */}
<form action="#" className="space-y-6 flex-grow">
<div className="space-y-2">
<label className="font-label-caps text-label-caps text-on-surface-variant ml-1" htmlFor="email">Email Identifier</label>
<input className="w-full bg-surface-container-low border border-outline-variant/50 rounded-lg px-4 py-4 font-body-md text-on-surface focus:ring-1 focus:ring-secondary focus:border-secondary transition-all placeholder:text-on-surface-variant/30 outline-none" id="email" placeholder="researcher@contextra.io" type="email"/>
</div>
<div className="space-y-2">
<div className="flex justify-between items-center px-1">
<label className="font-label-caps text-label-caps text-on-surface-variant" htmlFor="password">Secure Token</label>
<a className="font-label-caps text-[10px] text-tertiary hover:text-on-tertiary-fixed-variant transition-colors" href="#">Lost Access?</a>
</div>
<div className="relative">
<input className="w-full bg-surface-container-low border border-outline-variant/50 rounded-lg px-4 py-4 font-body-md text-on-surface focus:ring-1 focus:ring-secondary focus:border-secondary transition-all placeholder:text-on-surface-variant/30 outline-none" id="password" placeholder="••••••••••••" type="password"/>
<button className="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant/50 hover:text-on-surface-variant" type="button">
<span className="material-symbols-outlined text-[20px]">visibility</span>
</button>
</div>
</div>
<div className="flex items-center gap-3 pt-2">
<input className="w-4 h-4 rounded border-outline-variant text-secondary focus:ring-secondary/50" id="remember" type="checkbox"/>
<label className="font-body-md text-on-surface-variant text-[14px] cursor-pointer" htmlFor="remember">Maintain local session</label>
</div>
<button className="w-full bg-primary hover:bg-on-surface text-surface py-5 px-6 rounded-lg font-headline-md text-[18px] transition-all duration-300 shadow-xl shadow-primary/10 flex items-center justify-center gap-3 group mt-4" type="submit">
                        Begin Synthesis
